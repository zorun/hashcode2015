...
class Server(object):
    ...
    def output(self):
        if None in [self.pool, self.row, self.slot]:
            ...
            return '{} {} {}'.format(self.row, self.slot, self.pool)

    def __str__(self):
        return 'Server({}, {}, {})'.format(self.size, self.cpu, self.pool)
...
class DC(object):
    ...
    def place_server(self, server, pool):
        ...
        if optimal_place == (None, None):
            ...
            print("Keeping optimal place {}".format(optimal_place))
...
#            self.rows[i % self.nb_rows].add_server(s, i)
#            self.pools[i % self.nb_pools].add_server(s)
#            print(self.pools[i % self.nb_pools].score)
            #self.output()
